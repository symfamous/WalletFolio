...
class AnimationController {
  ...
  constructor(ctx: CanvasRenderingContext2D, size: number, particleColor: string) {
    this.ctx = ctx
    this.size = size
  ...
  private createStars() {
    ...
    // Use seeded random for star creation
...
export function SpiralAnimation() {
  ...
  useEffect(() => {
    ...
    const particleColor = isDark ? 'white' : '#0f172a'

    if (animationRef.current) {
      animationRef.current.destroy()
    }
    animationRef.current = new AnimationController(ctx, size, particleColor)
